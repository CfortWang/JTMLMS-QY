...
import { initColumn } from '@/views/system/dashboard/components'
    ...
    methods: {
        ...
        handleConfirm () {
            this.$refs[this.formName].validate(valid => {
                if (!valid) {
                    ActionUtils.warning('请选择栏目')
                    return
                }
                const item = { 'x': 0, 'y': 0, 'w': this.form.width, 'h': this.form.height, 'i': uuidv4() + '', alias: this.form.alias }
